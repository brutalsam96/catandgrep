...
void process_line(const char* line, regex_t* regex, const grep_flags* flags,
  ...
  while (regexec(regex, cursor, 1, pmatch, 0) == 0) {
    if (!flags->v_flag && !flags->c_flag && !flags->l_flag) {
      if (flags->n_flag && at_line_start && !flags->o_flag) {
      ...
      printf("\033[1;31m%.*s\033[0m", (int)(pmatch[0].rm_eo - pmatch[0].rm_so),
             cursor + pmatch[0].rm_so);
    }
  ...
  if (matched && !flags->v_flag) (*match_count)++;
  if (!flags->c_flag) {
    if (!matched && flags->v_flag) {
      if (!flags->h_flag && file_count > 1)
        printf("\033[35m%s\033[36m:\033[0m", filename);
      if (flags->n_flag && at_line_start) {
        printf("\033[32m%d\033[36m:\033[0m", actual_line_count);
    ...
    if (matched && !flags->v_flag && !flags->l_flag && !flags->o_flag)
      printf("%s", cursor);  // something might be wrong here
  } else if (!matched && flags->c_flag && flags->v_flag) {
    (*match_count)++;
  }
}
