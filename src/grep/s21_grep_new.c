...
void count_pattern(const char* filename, char** patterns, int pattern_count, int e_flag, int i_flag, int v_flag, int c_flag, int l_flag, int n_flag, int h_flag, int s_flag, int f_flag, int o_flag) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        printf("grep: %s: No such file or directory\n", filename);
        return;
    ...
    while (fgets(buffer, BUFSIZ, file) != NULL) {
        ...
        for (int i = 0; i < pattern_count; i++) {
            ...
            while ((match = strstr(current, lower_p)) != NULL) {
                ...
                // Move pointer forward past the match
                current = match + strlen(lower_p);
            }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < file_count; i++) {
        if (c_flag) {
            count_pattern(files[i], patterns, pattern_count, e_flag, i_flag, v_flag, c_flag, l_flag, n_flag, h_flag, s_flag, f_flag, o_flag);
        } else {
